...
void Autocomplete::search(string key, int& firstIndex, int& lastIndex)
{
	int match = binary_search(key); // finds a match
    	...
    	{	
       		leftIndex--; // move left toward 0 or the first term
    	}	
    	...
    	{
        	rightIndex++; // move towrard size()-1 or last term
    	}
	firstIndex = leftIndex + 1; // +1 to correc bound
	lastIndex = rightIndex - 1; // -1 to correect bound
}
...
SortingList<Term> Autocomplete::allMatches(string prefix)
{
	...
	for (int i = first; i <= last; i++) 
    	{
        	found.insert(terms[i]);
    	}
	if (found.size() > 0) // no point in sorting if it is 0
    	{
        	found.merge_sort(Term::compareByWeight);
    	}
    	return found;
}
